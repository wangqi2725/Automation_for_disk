    ...
    def _create_disk(self,asset):
        ...
        for disk_dict in disk_list:
            if not disk_dict.get('sn'):
                raise ValueError('unknown sn of disk')
            disk = models.Disk()
            disk.asset = asset
...
class UpdateExcMachine:
    """

    """
    def __init__(self,request,excmachine,status):
        self.request = request
        self.excmachine = excmachine
        self.status = status

    def UpdateStatus(self):
        """更新执行机状态"""
        self.excmachine.status = self.status
        self.excmachine.save()
