...
def collect():
    ...
    data['manufacturer'] = raw_data['Manfacturer']
...
def get_os_info():
    ...
    release = subprocess.Popen("lsb_release -a | grep 'Discription'",stdout=subprocess.PIPE,shell=True)
...
def get_ram_info():
    ...
    for line in raw_list:
        if line.startswith('Memory Device'):
            ...
            item_list = []
        else:item_list.append(line.strip())
            ...
            if len(data) == 2:
                ...
                if key == "Size":
                    if v.strip() != "No Module Installed":
                        ram_item_to_dic['capacity'] = v.strip()[0].strip()
